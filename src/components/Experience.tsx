    ...
    <section id="experience" className="px-6 py-28 md:px-12 md:py-40">
      ...
      <div className="relative mt-16 pl-8">
        ...
        {experience.map((item, i) => (
          <Reveal key={item.org} className={`relative ${i > 0 ? "mt-14" : ""}`}>
            ...
            <h3 className="mt-2 text-2xl leading-tight md:text-3xl">{item.role}</h3>
            <p className="text-sm text-muted">{item.org}</p>
            <p className="mt-3 max-w-md text-foreground/80">{item.description}</p>
          </Reveal>
        ))}
      </div>
    </section>
